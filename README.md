...
| 阶段            | 目标                                  | 输出                                                           |
| --------------- | ------------------------------------- | -------------------------------------------------------------- |
| 1. Spec         | 需求分析、技术架构设计、测试规划      | `docs/{feature}/spec.md`（含需求、架构、测试策略）             |
| 2. 生成接口代码 | 一次性生成完整代码(Schema + Handlers) | 完整的接口代码 + migration                                     |
| 3. 生成测试用例 | 基于接口类型生成可执行测试            | `__tests__/int.test.ts`                                        |
| 4-5. 循环优化   | 持续改进直到满足验收标准              | 通过验收的代码                                                 |
| 6. 模块文档     | 生成模块文档供后续 AI 开发维护使用    | `docs/{feature}/module.md`（模块关联文件、代码功能、技术要点） |
...
```
docs/{feature}/
├── spec.md    # Spec 文档（需求分析 + 技术架构 + 测试策略）
└── module.md  # 模块文档（供后续 AI 开发维护使用）
```
...
> 此文档供后续 AI 继续开发或维护当前模块时使用，用于快速了解当前模块关联了哪些文件及代码功能。
...
| 文件路径                                           | 用途          |
| -------------------------------------------------- | ------------- |
| `src/routes/admin/{feature}/{feature}.index.ts`    | 路由入口      |
| `src/routes/admin/{feature}/{feature}.routes.ts`   | 路由定义      |
| `src/routes/admin/{feature}/{feature}.handlers.ts` | 业务处理器    |
| `src/routes/admin/{feature}/{feature}.types.ts`    | 类型定义      |
| `src/routes/admin/{feature}/__tests__/int.test.ts` | 集成测试      |
| `src/db/schema/admin/{feature}/{entity}.ts`        | 数据库 Schema |
