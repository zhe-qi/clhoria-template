 ...
 */
export const systemParam = pgTable("system_param", {
  ...baseColumns,
  /** 参数键（唯一标识，如 "sms_sign"） */
  ...
  // key 字段的唯一约束已经包含了索引
  // 为 status 字段添加索引，用于查询启用的参数
  index("system_param_status_idx").on(table.status),
]);

/**
 * Select Schema（查询时使用）
 */
export const selectSystemParamSchema = createSelectSchema(systemParam, {
 ...
 * Insert Schema（插入时使用）
 */
export const insertSystemParamSchema = createInsertSchema(systemParam).omit({
  id: true,
  createdAt: true,
  createdBy: true,
  updatedAt: true,
