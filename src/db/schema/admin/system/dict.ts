...
export const systemDict = pgTable("system_dict", {
  ...
  index("system_dict_status_idx").on(table.status),
]);

/**
 * Select Schema（查询时使用）
 */
export const selectSystemDictSchema = createSelectSchema(systemDict, {
 ...
 * Insert Schema（插入时使用）
 */
export const insertSystemDictSchema = createInsertSchema(systemDict).omit({
  id: true,
  createdAt: true,
  createdBy: true,
  updatedAt: true,
