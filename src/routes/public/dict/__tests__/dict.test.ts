...
import dictRouter from "@/routes/public/dict/dict.index";
...
function createDictApp() {
  return createTestApp().route("/", dictRouter);
}

const client = testClient(createDictApp());
...
async function cleanupTestDicts(): Promise<void> {
  try {
    ...
    await db
      .delete(systemDict)
      .where(like(systemDict.code, "public_test_%"));
...
describe("public dict routes", () => {
  beforeAll(async () => {
    // 清理可能存在的遗留测试数据
    await cleanupTestDicts();

    // 创建测试字典
    await db.insert(systemDict).values(testDict);
  });
  ...
  describe("get /dict/{code} - get dict items by code", () => {
    it("should validate code format", async () => {
      const response = await client.dict[":code"].$get({
    ...
    it("should return 404 for disabled dict", async () => {
      ...
      await db.insert(systemDict).values({
        ...testDict,
        code: disabledCode,
    ...
    it("should use Redis cache on subsequent requests", async () => {
      ...
      // 第一次请求（应该从数据库查询）
      const response1 = await client.dict[":code"].$get({
        param: { code: testDictCode },
      });
      ...
      // 第二次请求（应该从缓存获取）
      const response2 = await client.dict[":code"].$get({
        param: { code: testDictCode },
      });
