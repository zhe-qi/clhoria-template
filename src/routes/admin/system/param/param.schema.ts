...
import { insertSystemParamSchema, selectSystemParamSchema } from "@/db/schema";
...
/** 创建参数 Schema */
export const systemParamCreateSchema = insertSystemParamSchema.extend({
  key: paramKeyField,
...
/** 更新参数 Schema */
export const systemParamPatchSchema = insertSystemParamSchema.extend({
  key: paramKeyField,
  value: z.string().min(1, "参数值不能为空").meta({ description: "参数值" }),
  valueType: z.enum([ParamValueType.STRING, ParamValueType.NUMBER, ParamValueType.BOOLEAN, ParamValueType.JSON]).optional().meta({ description: "参数值类型" }),
  name: z.string().min(1, "参数名称不能为空").max(128, "参数名称最多128个字符").meta({ description: "参数名称" }),
...
/** ID 参数 Schema */
export const systemParamIdParams = z.object({
  id: z.uuid("ID 必须是有效的 UUID").meta({ description: "参数ID" }),
});

/** 响应 Schema */
export const systemParamResponseSchema = selectSystemParamSchema;

/** 列表响应 Schema */
export const systemParamListResponse = z.array(systemParamResponseSchema);
