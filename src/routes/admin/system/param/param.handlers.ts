...
import type { SystemParamRouteHandlerType } from "./param.types";
...
export const create: SystemParamRouteHandlerType<"create"> = async (c) => {
  ...
  try {
    const [created] = await db.insert(systemParam).values({
      ...body,
      createdBy: sub,
    }).returning();
...
export const get: SystemParamRouteHandlerType<"get"> = async (c) => {
  ...
  const param = await db.query.systemParam.findFirst({
    where: eq(systemParam.id, id),
  });
...
export const remove: SystemParamRouteHandlerType<"remove"> = async (c) => {
  ...
  const [deleted] = await db
    .delete(systemParam)
    .where(eq(systemParam.id, id))
    .returning({ id: systemParam.id, key: systemParam.key });
