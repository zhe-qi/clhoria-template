...
import db from "@/db";
import { systemParam } from "@/db/schema";
import env from "@/env";
import { ParamValueType, Status } from "@/lib/enums";
import redisClient from "@/lib/redis";
import * as HttpStatusCodes from "@/lib/stoker/http-status-codes";
import { authorize } from "@/middlewares/authorize";
import systemParamRouter from "@/routes/admin/system/param/param.index";
...
function createSysParamApp() {
  return createTestApp()
    .use("/system/param/*", jwt({ secret: env.ADMIN_JWT_SECRET, alg: "HS256" }))
    .use("/system/param/*", authorize)
    .route("/", systemParamRouter);
}
...
async function cleanupTestParams(): Promise<void> {
  try {
    ...
    await db
      .delete(systemParam)
      .where(like(systemParam.key, "test_%"));
...
describe("system param routes", () => {
  ...
  describe("authentication & authorization", () => {
    it("should require authentication", async () => {
      const response = await client.system.param.$get(
        { query: {} },
        {},
      );
  ...
  describe("get /system/param - list params", () => {
    ...
    it("should support filtering by key", async () => {
      // 先创建一个参数
      await client.system.param.$post(
        {
          json: {
            ...testParam,
            key: "test_filter",
  ...
  describe("post /system/param - create param", () => {
    it("should validate required fields", async () => {
      const response = await client.system.param.$post(
        {
    ...
    it("should return 409 for duplicate key", async () => {
      ...
      // 创建第一个参数
      const response1 = await client.system.param.$post(
        {
          json: {
            ...testParam,
            key: duplicateKey,
      ...
      // 尝试创建重复的参数
      const response2 = await client.system.param.$post(
        {
          json: {
            ...testParam,
            key: duplicateKey,
  ...
  describe("get /system/param/{id} - get param", () => {
    let paramId: string;
    ...
    it("should validate UUID format", async () => {
      const response = await client.system.param[":id"].$get(
  ...
  describe("patch /system/param/{id} - update param", () => {
    let paramId: string;
    ...
    it("should update param fields", async () => {
      const response = await client.system.param[":id"].$patch(
        {
          param: { id: paramId },
          json: {
            name: "更新后的参数名称",
  ...
  describe("delete /system/param/{id} - delete param", () => {
    it("should validate UUID format", async () => {
      const response = await client.system.param[":id"].$delete(
        { param: { id: "invalid-uuid" } },
        { headers: getAuthHeaders(adminToken) },
      );
    ...
    it("should delete param successfully", async () => {
      // 先创建一个参数用于测试删除功能
      const createResponse = await client.system.param.$post(
        {
          json: {
            ...testParam,
            key: "test_delete",
      ...
      // 测试删除功能
      const deleteResponse = await client.system.param[":id"].$delete(
      ...
      // 验证参数已被删除
      const verifyResponse = await client.system.param[":id"].$get(
        { param: { id: deleteTestParamId } },
        { headers: getAuthHeaders(adminToken) },
      );
