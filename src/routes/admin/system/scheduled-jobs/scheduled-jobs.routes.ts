...
const ToggleStatusSchema = z.object({
  status: z.number().int().min(0).max(2).meta({ description: "新状态: 0=禁用 1=启用 2=暂停" }),
});
...
export const getExecutionStats = createRoute({
  ...
  responses: {
    [HttpStatusCodes.OK]: jsonContent(
      z.object({
        totalExecutions: z.number().meta({ description: "总执行次数" }),
        successfulExecutions: z.number().meta({ description: "成功执行次数" }),
        failedExecutions: z.number().meta({ description: "失败执行次数" }),
        averageDuration: z.number().meta({ description: "平均执行时间(毫秒)" }),
        lastExecution: z.date().nullable().meta({ description: "最后执行时间" }),
        successRate: z.number().meta({ description: "成功率(百分比)" }),
      }),
      "获取成功",
    ),
    [HttpStatusCodes.NOT_FOUND]: jsonContent(notFoundSchema, "任务不存在"),
...
export const getAvailableHandlers = createRoute({
  ...
  responses: {
    [HttpStatusCodes.OK]: jsonContent(
      z.array(
        z.object({
          name: z.string().meta({ description: "处理器名称" }),
          description: z.string().meta({ description: "处理器描述" }),
          filePath: z.string().meta({ description: "文件路径" }),
          isActive: z.boolean().meta({ description: "是否激活" }),
        }),
      ),
      "获取成功",
    ),
  ...
  responses: {
    [HttpStatusCodes.OK]: jsonContent(
      z.object({
        ...
        })).meta({ description: "任务统计" }),
        dailyStats: z.record(z.string(), z.object({
          total: z.number(),
          successful: z.number(),
          failed: z.number(),
        })).meta({ description: "每日统计" }),
      }),
      "获取成功",
    ),
    [HttpStatusCodes.UNPROCESSABLE_ENTITY]: jsonContent(
